...
float timeout;          /* the duration of the time from which the timer is started
                            and 'till it finishes */
...
struct Sender {
    int base;   // 확인응답 안 된 가장 오래된 패킷
    int nextseq;    // 전송될 다음 패킷의 순서번호
    //int window_size;    // 전송되었지만 아직 확인응답 안 된 패킷을 위해 허용할 수 있는 순서번호의 범위
    float estimated_rtt;
    int buffer_next;
...
struct Receiver {
    int expect_seq; // 수신할 것이라고 예상되는 패킷 순서번호
...
void send_window()
{
    ...
    if (A.nextseq > A.base)
    {
        printf("  send_window: Sender's window can not move forward. \n");
        return;
    }
    while (A.nextseq < A.buffer_next && A.nextseq < A.base + window_size)
    {
...
struct msg message;
{
    if (A.buffer_next - A.base >= BUFSIZE) {
        printf("  A_output: buffer is full. Drop the message: %s\n", message.data);
        return;
    }
...
struct pkt packet;
{
    // 수신된 checksum과 계산된 checksum이 다를시
    ...
    if (time > timeout)    // Timeout 체크
    {
        A_timerinterrupt(); // 재전송
        return;
    }
...
struct pkt packet;
{
    ...
    if (packet.seqnum < B.expect_seq)   // duplicate packet discard
    {
        printf("  B_input: not the expected seq. send NAK (ack=%d)\n", B.packet_to_send.acknum);
        tolayer3(1, B.packet_to_send);
        return;
    }
    else if (packet.seqnum > B.expect_seq)  // out of order packet을 buffer에 보관
    {
        printf("  B_input: recv packet (seq=%d): %s\n", packet.seqnum, packet.payload);
        memcpy(packet.payload, &(B.packet_buffer) + B.buffer_cnt * 20, 20);
        tolayer3(0, B.packet_to_send);
        B.buffer_cnt++;
        return;
    }
    ...
    if (B.buffer_cnt > 0)   // show out of order packet
    {
        printf("  B_input: send ACK (ack=%d)\n", B.expect_seq);
        printf("  B_input: Out of order packet: ");
    }
    for (int i = 0; i <= B.buffer_cnt; i++)
    {
        printf(" (seq=%d) ", B.packet_buffer[i]);
    }
    printf("\n");
    // 정상적인 PACKET 수신에 대한 ACK 응답 전송
    B.packet_to_send.acknum = B.expect_seq;
    B.packet_to_send.checksum = get_checksum(&B.packet_to_send);
    tolayer3(1, B.packet_to_send);
    B.expect_seq++;

    while (B.buffer_cnt > 0 && (B.packet_buffer[0].acknum == B.expect_seq))
    {
        tolayer5(1, B.packet_buffer[0].payload);
        B.expect_seq++;
        B.buffer_cnt--;
    }
...
init()                         /* initialize the simulator */
{
    ...
    scanf("%f", &lambda);
    printf("Enter the duration of the time from which the timer is started and 'til it finishes: ");
    scanf("%f", &timeout);
